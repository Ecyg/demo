...
// Remove potentially harmful input - Remove <script>, then just the word "script"
// $comment = str_replace(array("<script>"), '', $comment);
$name = str_replace(array("script"), '', $name);
        ...
        <form method="POST" action="?name=<?php echo $name; ?>">
            <div class="form-group">
                ...
                <textarea name="comment" id="comment" rows="4" placeholder="Share your thoughts..."><?php 
                    ...
                    // Think about a devious filter here
                    $blacklist = array('javascript:', 'onerror', 'onload', '<script', 'alert(');
                    $comment = str_replace($blacklist, '', $comment);
                    echo $comment; 
                ?></textarea>
            </div>
        ...
        <?php if(isset($_GET['search']) && !empty($_GET['search'])): ?>
            <div class="search-results">
                <h3>Search Results for: <?php 
                    ...
                    $search = str_ireplace("script", '', $search);
                    echo $_GET['search']; 
                ?></h3>
